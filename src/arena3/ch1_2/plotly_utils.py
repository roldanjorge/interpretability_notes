...
def _apply_text_to_imshow_fig(fig, tensor, text):
    if tensor.ndim == 2:
...
def _tensor_to_array(tensor):
    if isinstance(tensor, list):
        if isinstance(tensor[0], Tensor):
            return [to_numpy(tn) for tn in tensor]
        elif isinstance(tensor[0], list):
            return [np.array(tn) for tn in tensor]
        else:
            return np.array(tensor)
    return to_numpy(tensor)


def _set_hist_defaults(kwargs_post):
...
def _add_mean_lines(fig, arr, add_mean_line):
...
def imshow(tensor: Tensor, renderer=None, **kwargs):
    ...
    if text:
        _apply_text_to_imshow_fig(fig, tensor, text)
...
def bar(tensor, renderer=None, **kwargs):
    """ """
    arr: list | np.ndarray = _tensor_to_array(tensor)
    kwargs_post = {k: v for k, v in kwargs.items() if k in update_layout_set}
    kwargs_pre = {k: v for k, v in kwargs.items() if k not in update_layout_set}
    return_fig = kwargs_pre.pop("return_fig", False)
    names = kwargs_pre.pop("names", None)
...
def hist(tensor, renderer=None, **kwargs):
    ...
    return_fig = kwargs_pre.pop("return_fig", False)
    arr: list | np.ndarray = _tensor_to_array(tensor)
    _set_hist_defaults(kwargs_post)
    ...
    _add_mean_lines(fig, arr, add_mean_line)
    return fig if return_fig else fig.show(renderer=renderer)
...
def hists_per_comp(
    ...
    for ((row, col), title), in_dir in zip(
        titles.items(), out_by_component_in_unbalanced_dir, strict=False
    ):
        fig.add_trace(
            go.Histogram(
    ...
    fig.update_layout(
        ...
        barmode="overlay",
        legend={"yanchor": "top", "y": 0.92, "xanchor": "left", "x": 0.4},
